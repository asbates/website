...
This is the first of two posts on Bayesian linear regression with a proper prior. In this post I give an overview of the model and how to set up a proper prior distribution using expert knowledge. The next post describes how to sample from the posterior using Gibbs sampling and using Stan.
...
Now that we have some numbers but how do we translate that into parameters for $\beta$ and $\tau$? We actually asked questions about things that are easier to think about so we will need to apply some transformations to obtain the parameters we need.
